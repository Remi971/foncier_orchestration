...
def enveloppe_calculation(task_id: str, parameters: EnveloppeParamsDto):
    response = requests.post(f"{env.MICROSERVICE_SIG}/enveloppe", json={"task_id": task_id, "parameters": parameters})
    response.raise_for_status()